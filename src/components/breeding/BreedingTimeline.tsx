...
const Fill = ({
  ...
  type LoadPen = { units: LoadUnit[]; crossings: number };

  /**
   * La fournée découpée enclos par enclos.
   *
   * ## Pourquoi le paquet est le croisement, et pas la monture
   *
   * Les deux parents d'un accouplement doivent être **dans le même enclos** :
   * c'est là qu'ils se croisent. Remplir dix montures à la file sans regarder à
   * qui elles appartiennent couperait des couples en deux d'un enclos à l'autre,
   * et la moitié de la fournée ne pourrait pas s'accoupler. On empile donc des
   * paquets indivisibles — les deux parents d'un croisement, ou une fécondation
   * seule — et on ouvre un nouvel enclos dès que le paquet suivant ne rentre
   * plus.
   *
   * Un paquet peut ne porter qu'une monture : quand l'autre parent est déjà
   * fécond, il ne passe pas par l'enclos du tout.
   */
  const loadPens = (): LoadPen[] => {
    const byId = new Map(individuals.map((mount) => [mount.id, mount]));
    const bundles: LoadUnit[][] = [];

    for (const line of fill.couples) {
      for (let index = 0; index < line.count; index += 1) {
        const bundle: LoadUnit[] = [];
        for (const [sex, sideOf] of [
          ...
          // ce qui dépasse est à procurer. Voir `loadRows` avant #199.
    ...
    // Les fécondations sans croisement occupent une place chacune, dans le même
    // enclos que le reste : les compter ailleurs faisait dire « 38 montures »
    // sous « 40/40 places ».
    ...
    for (const bundle of bundles) {
      let pen = pens[pens.length - 1];
      if (!pen || pen.units.length + bundle.length > ENCLOS_SLOTS) {
        pen = { units: [], crossings: 0 };
        pens.push(pen);
      }
      pen.units.push(...bundle);
      if (!bundle[0].banked) pen.crossings += 1;
    }
    return pens;
  };
                ...
                return (
                  ...
                  >
                    <div className="flex flex-wrap items-baseline gap-2">
                      ...
                      <span className="text-[10px] text-dark-500 tabular-nums">
                        {pen.units.length}/{ENCLOS_SLOTS} places · {pen.crossings} croisement
                        {pen.crossings > 1 ? 's' : ''}
                      </span>
                    </div>
