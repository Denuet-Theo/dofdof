-- Rend le filtre « seulement les ingrédients de craft » exploitable.
--
-- Tel qu'écrit depuis 20260802210000, le filtre testait chaque ligne de drop
-- contre la table des recettes :
--
--     and (not p_crafted_only
--          or exists (select 1 from public.dofus_recipes r
--                     where r.ingredient_ids @> array[d.object_id]))
--
-- Coincé dans un `or` avec le paramètre, cet `exists` ne peut pas devenir un
-- semi-join : le planificateur en fait un SubPlan corrélé, réexécuté pour
-- chacune des ~40 000 lignes de `dofus_drops`. Et sur une table de 4 858
-- recettes tenant en quelques dizaines de pages, il préfère le parcours
-- séquentiel au GIN `dofus_recipes_ingredient_ids_gin`, dont le coût de
-- démarrage ne s'amortit pas — soit de l'ordre de 10⁸ comparaisons de tableaux.
-- La requête dépassait le statement_timeout : cocher la case ne rendait jamais
-- de réponse, seulement l'erreur générique de la route.
--
-- On déplie donc les ingrédients **une seule fois**, en amont. L'ensemble tient
-- en quelques milliers d'ids, le `in` devient un SubPlan haché, et le test
-- redevient constant par ligne. Le reste de la fonction est identique à
-- 20260802220000.
...
-- Les objets qui entrent dans au moins une recette, dépliés une seule fois.
--
-- `where p_crafted_only` rend la CTE vide — donc gratuite — quand la case n'est
-- pas cochée. Le `materialized` est explicite parce que c'est là tout l'intérêt
-- de la manœuvre : la liste doit être construite une fois, pas réévaluée sous
-- le `in` ligne à ligne.
craft_ingredients as materialized (
  select distinct ingredient.object_id
  from public.dofus_recipes r
  cross join lateral unnest(r.ingredient_ids) as ingredient(object_id)
  where p_crafted_only
),
...
valued as (
  ...
  where d.percent_max >= p_min_percent
    ...
    and (
      not p_crafted_only
      or d.object_id in (select object_id from craft_ingredients)
    )
)
