-- Deux corrections à `price_suggestions` (20260802120000).
--
--  1. Le bloc `cost_driver` ne retenait que les recettes dont les *ingrédients*
--     étaient tous tarifés. Sans prix de vente il n'y a aucune marge à fausser,
--     donc rafraîchir l'ingrédient dominant d'une telle recette ne corrige rien
--     à l'écran — ces recettes sont déjà le sujet du bloc `ready_recipe`. On
--     exige désormais une recette complète au sens de `recipeHasAllPrices`.
--
--  2. `super_type_id` est remonté, pour que le client puisse afficher l'hôtel
--     de vente de l'item (src/lib/dofus/hdv.ts).
--
-- Nouvelle migration plutôt qu'une réécriture de la précédente : celle-là est
-- déjà mergée, donc enregistrée comme appliquée — la modifier sur place ne la
-- rejouerait jamais. Et un `create or replace` seul ne suffirait pas, Postgres
-- refusant de changer le type de retour d'une fonction existante.
...
b3 as (
  ...
  from (
    select
      i.id, i.name_fr, i.img, i.super_type_id, i.has_recipe, pr.price, pr.updated_at,
...
-- affecté par le remplacement de la fonction.
